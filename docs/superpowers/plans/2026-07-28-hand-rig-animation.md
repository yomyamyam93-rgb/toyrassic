...
## ★스윙 클립은 정확히 1.00초로 만든다 (2026-07-28 사용자 결정)

길이를 통일하지 않으면 공속을 맞출 수 없다. 1초로 고정하면:

```
실제 스윙 시간 = 1.00초 ÷ 공속
```

애니메이터 상태의 Speed 를 공속에 물리면, 민첩·버프가 오를 때 클립을 다시 만들지
않고 모든 무기가 같은 비율로 빨라진다. 타격 이벤트도 클립 안의 '비율 위치' 라
자동으로 따라온다. DPS = 데미지 × 공속 으로 단순해진다.

**무기별 무게감은 클립 길이가 아니라 무기의 공속 배수로 표현한다.**
(도끼를 느리게 = 클립을 길게 만드는 게 아니라 공속 배수 0.7)

`PlayerGather` 의 스윙 지속시간도 같은 `1.00 ÷ 공속` 을 따라야 판정과 모션이
어긋나지 않는다 — Task 6 에서 함께 맞춘다.

권장 배분 (1초 안의 비율):
```
0:00 ~ 0:35  들어올리기 (천천히)
0:35 ~ 0:50  내려치기 (순간)     ← 타격 이벤트 0:48 쯤
0:50 ~ 1:00  마무리
```
